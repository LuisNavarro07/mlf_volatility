...
tempfile placebosforinference
save `placebosforinference'
restore 

/// Merge 
merge m:1 id using `placebosforinference', keep(match master) nogen
keep if survival == 1
/// Remove Volatile Series 
drop if id == 148
save "${tem}\placeboempiricaldistributionsp.dta", replace 
...
qui use "${tem}\placeboempiricaldistributionsp.dta" , clear  
...
drop if id == 29 
save "${tem}\synth_placebos_sp.dta", replace 
**************************************************************
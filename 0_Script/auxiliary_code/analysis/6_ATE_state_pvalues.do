...
matrix define M=J($rows ,5,.)
matrix colnames M = "ate" "cohend" "pval" "rmse" "id"
...
forvalues j = 1(1)$rows { 
	...
	*qui drop if cohend < $cohend_cutoff
	...
	qui kdensity absate, recast(line) lcolor(black) lwidth(medthin) lpattern(solid) xline($atetreat, lcolor(maroon) lpattern(dash)) xtitle("") title("`title`i'': p = $pvalue", pos(11) size(small)) ytitle("") xlabel(#10, angle(0) labsize(small)) ylabel(#10, angle(0) labsize(small)) name(gr`j',replace) xscale(range(0 $ate)) legend(off)
	restore 
}

grc1leg gr1 gr2 gr3 gr4, legendfrom(gr1) name(grcomb1,replace) $combopts 
...
label variable M2 "Cohen-D"
label variable M3 "P-Value"
label variable M4 "RMSE"
rename (M1 M2 M3 M4 M5) (ate cohend pval rmse id)
gen category = ""
...
drop id
order category
format ate cohend pval rmse %12.4fc

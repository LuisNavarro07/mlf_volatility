...
*** Script: Run Baseline Model: Fixed Credit Rating 
...
********************************************************************************
/// 2. Placebo Estimation: Define program that adjusts placebos using the mean and sd from data 
/// run the program defined above 
use "${cln}/synth_clean_fixedcr.dta", clear 
drop if month_exp <= 3
do "${cod}/code_analysis/8_3_PlaceboOutcomeRC_FixedCR.do"
tempfile synth_placebos
save "${tem}/synth_placebos.dta", replace 

/// 4. Placebo Empirical Distribution: This is not Estimated Again.
use "${tem}/placeboap_rc_fixedrating.dta", clear 
/// Goodness of Fit 
preserve 
// Survival Test 
...
tempfile placebosforinferencerce
save `placebosforinferencerce' , replace 
restore 

use "${tem}/synth_placebos.dta", clear 
merge m:1 id using `placebosforinferencerce', keep(match master) nogen
keep if survival == 1
/// Save data 
...
grc1leg gr3 gr2 gr1 gr4, legendfrom(gr2) name(grcombrcfixedrating,replace) $combopts 
graph display grcombrcfixedrating, ysize(80) xsize(100) scale(.9)
graph export "${oup}/Synth_GraphAggregated_RC_fixedrating_${tr_eff_window}.pdf", $export
********************************************************************************
/// 5. ATE and Pvalues 
use "${tem}/synth_treated.dta", clear 
tab fileid
...
graph export "${oup}/ate_dens_fixedcr.pdf", $export 
...
list 
texsave * using "${oup}/ATE_Results_RC_fixedrating_${tr_eff_window}.tex", replace  decimalalign nofix 
...
preserve 	
use "${tem}/ATE_Results_Full_RC.dta", clear
...
graph export "${oup}/SmokeCombinedOutRC_fixedrating_${tr_eff_window}.pdf", $export 
*******************************************************************************

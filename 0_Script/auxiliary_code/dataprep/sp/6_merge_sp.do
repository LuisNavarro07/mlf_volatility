/// Do File for using the sp yield data 
/// Merge the Data
...
*******************************************************************************
/// Merge Names to Store them later 
merge m:1 sec_id using "${tem}\varnames.dta", keep(match master) nogen 
/// Drop outcomes from bloomberg 
drop if sec_id <=8  
gen bl = 1 
append using "${tem}\spyields.dta"
replace bl = 0 if bl == . 
replace sec_id = sec_id + 42 if bl == 1
sort sec_id date 
...
gcollapse (mean) yield (min) min (max) max (sd) sd, by(wofd sec_id name)
sort sec_id wofd 

gen range = max - min 


save "${cln}\financial_market_volatility_sp.dta", replace 

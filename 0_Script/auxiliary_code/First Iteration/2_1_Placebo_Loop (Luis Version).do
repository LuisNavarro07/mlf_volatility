...
use "${tem}\TheFile.dta", clear 
	...
	*display "${levels}"
	tempfile SCM_placebo_`t'_`i'
	synth v ${levels}, trunit(${t_id}) trperiod(${tr_dt1}) keep(`SCM_placebo_`t'_`i'')
	...
	*preserve	
		...
		*save 	`scm_`t''
		...
		order id fileid ate 
		drop if _n > 1 
		save "${tem}\placebo_b`t'_unit`i'.dta", replace 
	*restore	
